...
file found in this priority order:
    ...
    1. Path in the ``DOORCAM_CONFIG`` environment variable (if set).
    2. ``/etc/doorcam/config.yml``
...
def _candidate_paths(env: dict[str, str] | None = None) -> list[Path]:
    ...
    paths: list[Path] = []
    env_path = env.get("DOORCAM_CONFIG")
    if env_path:
...
def load_config(
    ...
    Args:
        ...
        env: Environment mapping to inspect for ``DOORCAM_CONFIG``. Defaults to
