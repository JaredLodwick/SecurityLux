...
# Pi acting as both hub and viewer), re-run the installer once per component.
...
choose_mm_position () {
    ...
    for i in "${!MM_POSITIONS[@]}"; do
        printf "  %2d) %s\n" "$((i + 1))" "${MM_POSITIONS[$i]}" >&2
    done
    echo >&2
    printf "Enter 1-%s [1]: " "${#MM_POSITIONS[@]}" >&2
    read -r choice
    choice="${choice:-1}"
    if [[ ! "$choice" =~ ^[0-9]+$ ]] || (( choice < 1 || choice > ${#MM_POSITIONS[@]} )); then
        echo "ERROR: invalid position choice '$choice'." >&2
        exit 1
    fi
    echo "${MM_POSITIONS[$((choice - 1))]}"
}
...
# Sanity: we're running from a real SecurityLux checkout.
if [[ ! -d "$REPO_DIR/hub" && ! -d "$REPO_DIR/camera_node" && ! -d "$REPO_DIR/mm_module" ]]; then
    echo "ERROR: $REPO_DIR doesn't look like a SecurityLux checkout." >&2
...
MM_POSITION=""          # used by viewer only
...
# Generated by SecurityLux install.sh.
...
EOF
        ...
        MM_POSITION=$(choose_mm_position)
        echo
        echo "==> Symlinking MMM-SecurityLuxDisplay into MagicMirror..."
        bash "$REPO_DIR/mm_module/install.sh" "$MM_PATH" "$HUB_URL" "$CAM_ID" "$MM_POSITION"
        ;;
...
The hub is running as the systemd service 'security-lux-hub'.
...
Logs:           journalctl -fu security-lux-hub
Config:         sudoedit /etc/security-lux-hub/config.yml
                (then: sudo systemctl restart security-lux-hub)
...
The MMM-SecurityLuxDisplay module is symlinked into MagicMirror at:
    $MM_PATH/modules/MMM-SecurityLuxDisplay

MagicMirror config was updated at:
    $MM_PATH/config/config.js
