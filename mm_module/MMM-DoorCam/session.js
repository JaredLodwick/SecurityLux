...
class SessionManager {
    ...
    _startSession(camId, t, confidence) {
        ...
        try {
            ...
            this.log.warn(`[MMM-DoorCam] recorder failed to start for ${camId}: ${err && err.message}`);
            recorder = null;
        }
        ...
        try {
            ...
            this.log.warn(`[MMM-DoorCam] store.insertSession failed for ${camId}: ${err && err.message}`);
        }
        ...
        this.log.info(`[MMM-DoorCam] session started cam=${camId} id=${rowId ?? "?"} conf=${confidence.toFixed(2)}`);
    }
    ...
    async _endSession(camId, t, reason) {
        ...
        if (sess.recorder) {
            try {
                ...
                this.log.warn(`[MMM-DoorCam] recorder stop failed for ${camId}: ${err && err.message}`);
            }
        }
        ...
        if (drop) {
            this.log.info(`[MMM-DoorCam] session dropped cam=${camId} reason=below-min duration=${duration}ms`);
            if (sess.rowId) {
        ...
        if (sess.rowId) {
            try {
                ...
                this.log.warn(`[MMM-DoorCam] store.finalizeSession failed for ${camId}: ${err && err.message}`);
            }
        }

        this.log.info(
            `[MMM-DoorCam] session ended cam=${camId} id=${sess.rowId ?? "?"} ` +
    ...
    _logEndError(err) {
        this.log.warn(`[MMM-DoorCam] session end error: ${err && err.message}`);
    }
}
