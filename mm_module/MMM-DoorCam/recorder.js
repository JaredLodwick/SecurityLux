...
class Recorder {
    ...
    start() {
        ...
        this.log.info(`[MMM-DoorCam] recording ${this.cam.id} -> ${relPath} (${codec})`);

        try {
            this.proc = spawn("ffmpeg", args, { stdio: ["pipe", "ignore", "pipe"] });
        } catch (err) {
            this.log.error(`[MMM-DoorCam] failed to spawn ffmpeg: ${err.message}`);
            this.proc = null;
            return;
        }
        ...
        this.proc.stderr.on("data", (chunk) => {
            ...
            for (const line of lines) {
                if (/error|invalid|failed|cannot|no such/i.test(line)) {
                    this.log.warn(`[MMM-DoorCam] ffmpeg(${this.cam.id}): ${line.trim()}`);
                }
            }
        });
        ...
        this.proc.stdin.on("error", (err) => {
            ...
            else this.log.warn(`[MMM-DoorCam] ffmpeg stdin error: ${err.message}`);
        });
        ...
        this._exitPromise = new Promise((resolve) => {
            this.proc.once("exit", (code, signal) => {
                ...
                if (code !== 0 && signal !== "SIGINT") {
                    this.log.warn(`[MMM-DoorCam] ffmpeg(${this.cam.id}) exited code=${code} signal=${signal}`);
                }
                resolve({ code, signal });
            });
        });
    ...
    _scheduleTick(delay) {
        ...
        this._tickTimer = setTimeout(() => this._tick().catch((err) => {
            this.log.error(`[MMM-DoorCam] recorder tick error: ${err && err.message}`);
        }), delay);
    }
    ...
    async _tick() {
        ...
        try {
            ...
            this.log.warn(`[MMM-DoorCam] ffmpeg write failed: ${err.message}`);
            this.stopped = true;
            return;
        }
        ...
        if (drainedSync === false) {
            try {
                ...
                this.log.warn(`[MMM-DoorCam] ffmpeg backpressure stall: ${err.message}`);
                this.stopped = true;
                return;
            }
        }
