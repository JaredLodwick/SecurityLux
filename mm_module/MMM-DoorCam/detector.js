...
const DEFAULT_MODEL_DIR = "~/.mm-doorcam/models";
...
class Detector {
    ...
    _spawnWorker() {
        ...
        try {
            ...
            this.log.error(`[MMM-DoorCam] failed to spawn detector worker: ${err.message}`);
            this.worker = null;
            return false;
        }
        ...
        this.worker.on("error", (err) => {
            this.log.error(`[MMM-DoorCam] detector worker error: ${err && err.message}`);
        });
    ...
    _onWorkerMessage(msg) {
        ...
        if (msg.kind === "ready") {
            ...
            this.log.info(`[MMM-DoorCam] detector worker ready (model: ${path.basename(this.modelPath)})`);
            return;
        }
        if (msg.kind === "result") {
        ...
        if (msg.kind === "error") {
            if (msg.fatal) {
                this.log.error(`[MMM-DoorCam] detector worker fatal: ${msg.message}`);
                this.workerReady = false;
            }
            if (msg.jobId) {
                const job = this.pendingJobs.get(msg.jobId);
    ...
    _onWorkerExit(code) {
        ...
        if (this._restartHistory.length >= WORKER_RESTART_MAX) {
            this.log.error(`[MMM-DoorCam] detector worker died ${WORKER_RESTART_MAX} times in ${WORKER_RESTART_WINDOW_MS / 1000}s; giving up`);
            return;
        }
        this._restartHistory.push(now);
        this.log.warn(`[MMM-DoorCam] detector worker exited code=${code}; restarting`);
    ...
    _scheduleNextTick(camId) {
        ...
        entry.timer = setTimeout(() => this._tick(camId).catch((err) => {
            this.log.warn(`[MMM-DoorCam] detector tick error: ${err && err.message}`);
        }), intervalMs);
    ...
    _flushLatencyStats() {
        ...
        this.log.info(`[MMM-DoorCam] detector latency p50=${p50}ms p95=${p95}ms n=${sorted.length}`);
        this._latencySamples = [];
    }
    ...
    async _ensureModel() {
        ...
        if (fs.existsSync(targetPath)) {
            ...
            this.log.warn(`[MMM-DoorCam] cached model ${fileName} sha256 mismatch; re-downloading`);
            try { fs.unlinkSync(targetPath); } catch (_) { /* ignore */ }
        }

        this.log.info(`[MMM-DoorCam] downloading detection model: ${url}`);
        ...
        if (expectedSha) {
            ...
            this.log.warn("[MMM-DoorCam] detection.modelSha256 not set; skipping integrity check");
        }
