 ...
 *
 * Requires `better-sqlite3` to be installed (the install.sh script handles
 * this on the hub Pi; for local runs install it in MagicMirror's root and
 * make the symlink, or install it directly into this directory).
 */
...
function tmpDb() {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mm-doorcam-store-"));
