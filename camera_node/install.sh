...
#   5. Bootstraps /etc/camera-node/config.yml from config.example.yml (only if absent)
#   6. Renders a systemd unit (camera-node.service) with this user + this path
...
if ! command -v systemctl >/dev/null 2>&1; then
    ...
    echo "    cd \"$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)\" && .venv/bin/python -m camera_node" >&2
    exit 1
fi
...
SYSTEM_CONFIG_DIR=/etc/camera-node
...
echo "==> Reloading systemd and (re)starting camera-node.service..."
systemctl daemon-reload
systemctl enable -q camera-node
systemctl restart camera-node
...
echo
if systemctl is-active --quiet camera-node; then
    echo "[OK] camera-node.service is running."
    echo
    echo "Live status:"
    systemctl status camera-node --no-pager --lines=5 || true
    echo
    cat <<EOF
----------------------------------------------
 Useful commands
----------------------------------------------
 Live logs:        journalctl -fu camera-node
 Recent logs:      journalctl -u camera-node -n 100 --no-pager
 Service status:   systemctl status camera-node
 Restart:          sudo systemctl restart camera-node
 Stop / disable:   sudo systemctl disable --now camera-node
 Edit config:      sudoedit $SYSTEM_CONFIG_FILE   (then restart)
EOF
else
    echo "[FAIL] camera-node.service is not running. Recent logs:" >&2
    journalctl -u camera-node -n 50 --no-pager >&2
    exit 1
fi
